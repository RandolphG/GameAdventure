export class GamepadInput {
  ...
  prevRawGamepadTypes: (Gamepad | null)[];
  supported: (Gamepad | null)[];
  ticking: boolean;
  ...
  constructor() {
    this.supported = navigator.getGamepads && navigator.getGamepads();
    this.ticking = false;
    // this.pan = new THREE.Vector3(0, 0, 0);
    // this.roll = new THREE.Vector3(0, 0, 0);

    // Recommended dead zones for Xbox One controller
  ...
  init() {
    console.log("GamepadInput init", this);
  ...
  pollGamepads() {
    let rawGamepads = navigator.getGamepads && navigator.getGamepads();
    if (rawGamepads) {
      this.gamepads = [];
